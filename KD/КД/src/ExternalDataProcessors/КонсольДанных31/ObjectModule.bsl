...
//В модуле многие методы неявно вызываются через процедуру ВызватьАлгоритмОбъект,
//фактически являясь программным интерфейсом, поэтому разделы сразу разбиты по тематике
//и область ПрограммныйИнтерфейс отсутствует
...
#Область КОНСОЛЬ_ДАННЫХ

#Область ВЫЗОВ_АЛГОРИТМОВ

#Область ВСПОМОГАТЕЛЬНЫЕ_МЕТОДЫ_ПРИ_ВЫЗОВАХ
...
//+ Вызывается через ВызватьАлгоритмОбъект. Дополняет структуру СПрм из текста кода.	
Процедура ДополнитьПараметрыПоКоду(Текст_Кода, СПрм)
	...
	Пока ТекПоз <> 0 Цикл
		Пока ЗначениеЗаполнено(ТекСим) И ТекСим <> ";" И ТекСим <> "." Цикл
...
// Возвращаемое значение:
//   	Массив из Структура - с полями
//   			*Тип - Строка -
//   				*Форма (Код, выполняемый в обычной форме или в управляемой на клиенте),
//   				*Клиент(выполняемый в отдельной форме на клиенте),
//   				*Сервер (выполняемый в УФ на сервере)
//   				*Объект (выполняемый в модуле обработки),
...
//Вызывает алгоритм в модуле объекта
//Параметры:
//	Код_______ - Строка - Текст кода алгоритма
//	Прм - Структура - Параметры выполнения. По умолчанию Неопределено = создаётся новая.
//	Подготовка - Булево - Необходимость добавления в код параметров для бесшовного выполнения.		
Процедура ВызватьАлгоритмОбъект(Код_______, Прм = Неопределено, Подготовка = Истина) Экспорт
	...
	Если Подготовка Тогда
		Код_К_Выполнению = ДобавитьПараметрыВВыполняемыйКод(Код_______, Прм);
	Иначе
...
// Параметры:
//	ЗапросИлиЕгоТекст - Запрос, Строка - Запрос или тескт для его создания
//	Параметры_ - Структура - Структура параметров обработки, дополняемая параметрами заппроса, по умолчанию НЕопределено
//	МВТ - МенеджерВременныхТаблиц - используется при создании запроса из текста, по умолчанию Неопределено
...
// Формирует запрос по тексту и параметрам
// Параметры:
//	Текст - Строка - Текст запроса
//	Параметры_ - Структура - для вставки в запрос как параметры, по умолчанию Неопределено
//	МВТ - МенеджерВременныхТаблиц - для вставки в запрос, по умолчанию Неопределено = сздаётся новый, если установлен ИспользоватьМВТ
// Возвращаемое значение:
//	Запрос
...
// Параметры:
//	Табдокумент - ТабличныйДокумент - Приёмник результатов СКД
//	СКД - СхемаКомпоновкиДанных - для исполнения
//	НастройкиСКД - НастройкиКомпоновкиДанных - Настройки для выполнения. По умолчанию Неопределено - будут взяты из СКД.
...
Функция ИмяФайлаНастроекПоУмолчанию() Экспорт
	Возврат ИмяКаталога(ИспользуемоеИмяФайла) + "\НастройкиКД\Настройки\ПоУмолчанию.нст3"
...
#Область МЕНЮ_НАСТРОЙКИ_КОМАНДЫ
	
#Область ДЕРЕВО_КОМАНД

//Добавляет строку дерева значений в заданное место
// Параметры:
//	Куда - ДеревоЗначений, СтрокаДереваЗначений - приёмник (узел дерева)
//	Позиция - Число - номер строки,	после которой будет вставлена новая
//	Имя - Строка
//	Представление_ - Строка
//	Картинка_ - Картинка - для вставляемой строки
// Возвращаемое значение:
//	СтрокаДереваЗначений - добавленная
...
//Ищет по пути строку в дереве команд
// Параметры:
//	Дерево - ДеревоЗначений - Место поиска
//	Путь - Строка - Путь к строке дерева по именам строк
//	Уровень - Число - Уровень узла дерева (номер искомой позиции в пути)
//	Уровней - Число - Уровней к проверке/поиску
// Возвращаемое значение:
//	СтрокаДереваЗначений, Массив из СтрокаДереваЗначений - найденные строки
...
// Возвращаемое значение:
//	Строка, Картинка - картинка или путь к ней (см. Путь)
...
// Параметры:
//  УдаляемыйИД  - Булево - ИД удаляемой ветки
//  СПодменю  - Булево - Удалить дочерние подменю
//  АдресДереваМеню  - Строка - Адрес дерева во временном хранилище
// Возвращаемое значение:
//	ДеревоЗначений - дерево меню
...
//Возвращает загруженные из хранилищ меню, настройки и команды по итогам настроек с наложением на заданные или без
//Параметры:
...
//		*ИменаНастроек - Неопределено, СписокЗначений из Строка - Список имён сохранённых настроек 
//		*Команды - Структура из см.СтандартныйСписокКоманд
//		*Меню - ДеревоЗначений из см.СтандартныеКомандныеПанели
//		*Настройки - Структура из см.СтандартныйСписокНастроек
...
//Записывает команду в хранилище. Вызывается через ВызватьАлгоритмОбъект.
// Параметры:
//	ПараметрыСохранения - Структура из Произвольный - см.
...
//Читает/устанавливает наличие согласия с авторскими правами
// Параметры:
//	ТолькоПринято - Строка - флаг, что соглашение принято, но информация ещё не записана
// Возвращаемое значение:
...
// Параметры:
//	ИмяУровня - Строка - Имя узла дерева требуемого уровня
//	ЧитатьСохранённые - Булево - Следует ли читать сохранённые ранее пользователем команды
// Возвращаемое значение:
//	ДеревоЗначений
	...
	Если ЧитатьСохранённые Тогда
		...
		Для каждого СохрК Из СохранённыеКоманды Цикл
			...
			Если ТекРодитель = Неопределено Тогда
				...
				Сбщ.Текст = "Неудачная попытка	создания команды " + СохрК.Значение.Имя + "!";
				Сбщ.Сообщить();
			Иначе
